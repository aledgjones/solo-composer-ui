...
export function drawKeySignature(x: number, y: number, clef: Clef, key: KeySignature, staveKey: string) {
    ...
    if (key.offset < 0) {
        ...
        const pattern = key_signature_patterns[clef_pitch][clef.offset][Accidental.Flat];
        for (let i = 0; i > key.offset; i--) {
            instructions.push(
                buildText(`${key.key}-${staveKey}-${i}`, styles, x + i * -1, y + 0.5 * pattern[i * -1], glyph)
            );
        }
    }
    ...
    if (key.offset > 0) {
        ...
        const clef_pitch = pitch_to_parts(clef.pitch)[0];
        const pattern = key_signature_patterns[clef_pitch][clef.offset][Accidental.Sharp];
        for (let i = 0; i < key.offset; i++) {
            instructions.push(buildText(`${key.key}-${staveKey}-${i}`, styles, x + i, y + 0.5 * pattern[i], glyph));
        }
    }

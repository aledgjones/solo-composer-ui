...
export const timeSignatureActions = {
  create: (
    ...
    const time_signature = create_time_signature(tick, beats, beat_type, draw_type, groupings);
    store.update((draft, state) => {
      ...
      if (overflow > 0) {
        ...
        draft.score.flows.by_key[flow_key].length = flow.length + (ticks_per_bar - overflow);
      }

      // move the future time signatures by the offset
      for (let i = tick + 1; i < flow.length; i++) {
        const entries = get_entries_at_tick(i, flow.master, EntryType.TimeSignature);
        entries.forEach((entry) => {
          move_entry(draft.score.flows.by_key[flow_key].master, entry.key, i + (ticks_per_bar - overflow));
        });
      }
    });

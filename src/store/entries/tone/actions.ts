...
export const toneActions = {
  ...
  create: (
    ...
    store.update((draft, state) => {
      insert_entry(
        draft.score.flows.by_key[flow_key].staves[stave_key].tracks.by_key[
          track_key
        ],
        tone
      );
  ...
  update: (
    ...
    store.update((draft) => {
      ...
      const tone = draft.score.flows.by_key[flow_key].staves[stave_key].tracks
        .by_key[track_key].entries.by_key[entry_key] as Tone;

...
export function create_instrument(id: string): Instrument {
    const { type: instrument_type, long_name, short_name, staves } = get_def(id);
    return {
        key: shortid(),
        id,
...
export function get_full_path_from_partial(selection: string[]): { path: string[]; id: string } {
...
export function instrumentName(instrument: Instrument, count_style: AutoCountStyle, count?: number) {
...
export function getCounts(
    ...
    const counts = players.order.reduce<InstrumentCountsTotals>((output, playerKey) => {
